 ...
 * 	have been renamed to use an "portable_" prefix, the function calls in the
 ...
 *    	#endif
 *    	#if !HAVE_VASPRINTF
 *    	int portable_vasprintf(char **, const char *, va_list);
 *    	#endif
 *    	#if !HAVE_ASPRINTF
 *    	int portable_asprintf(char **, const char *, ...);
 *    	#endif
 *    	#endif
 *
...
int
portable_vsnprintf(char *str, size_t size, const char *format, va_list args)
{
	LDOUBLE fvalue;
	INTMAX_T value;
	...
	while (ch != '\0')
		...
		case PRINT_S_CONV:
			...
			case 'p':
				 ...
				 */
				if ((strvalue = va_arg(args, const char*)) == NULL)
					/*
...
again:
	 ...
	 */
	mask = static_cast<unsigned long>(mypow10(precision));
	/*
...
cast(LDOUBLE value)
{
	...
	result = static_cast<UINTMAX_T>(value);
	/*
...
int
portable_vasprintf(char **ret, const char *format, va_list ap)
{
	size_t size;
	int len;
	va_list aq;
	...
	VA_COPY(aq, ap);
	len = portable_vsnprintf(NULL, 0, format, aq);
	VA_END_COPY(aq);
	if (len < 0 || (*ret = reinterpret_cast<char*>(malloc(size = len + 1))) == NULL)
		return -1;
	return portable_vsnprintf(*ret, size, format, ap);
}
#endif	/* !HAVE_VASPRINTF */
...
#if HAVE_STDARG_H
int
portable_snprintf(char *str, size_t size, const char *format, ...)
#else
int
portable_snprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	char *str;
	...
	len = portable_vsnprintf(str, size, format, ap);
	va_end(ap);
	return len;
}
#endif	/* !HAVE_SNPRINTF */
...
#if HAVE_STDARG_H
int
portable_asprintf(char **ret, const char *format, ...)
#else
int
portable_asprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	char **ret;
